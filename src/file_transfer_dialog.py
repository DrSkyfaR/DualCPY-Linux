...
# Windows-only flag to hide the adb console window; ignored (0) elsewhere.
...
class FileTransferDialog:
    ...
    def __init__(self, parent, adb_bin, serial):
        ...
        # Worker threads (listing, transfers, previews, SD detection) post UI
        # updates here; drained on the Tk thread by _poll. Tcl/Tk is not
        # thread-safe, so cross-thread after() is unsafe (raises on Linux).
    ...
    def _ui(self, fn, *args):
        """Marshal a UI update from a worker thread onto the Tk thread."""
    ...
    def _poll(self):
        try:
            ...
            logger.exception("File transfer UI handler failed")
        try:
            self.window.after(50, self._poll)
        except Exception:
    ...
    def _build_window(self):
        ...
        self._win_frame = self._build_pane(body, col=0, label="Local PC",
    ...
    def _load_image_win(self, path):
        try:
            ...
            self._ui(self._apply_thumb, tk_img)

        except Exception as e:
            logger.debug(f"Win preview failed for {path}: {e}")
            self._ui(self._set_thumb_icon, "warning", "Preview\nfailed")
    ...
    def _detect_sd_cards(self):
        ...
        for name in out.split():
            ...
            self._ui(self._add_sd_pill, path, short)
    ...
    def _populate_drive_buttons(self, parent):
        ...
        # Then the "drives": Windows drive letters, or Linux roots/mount points.
        if sys.platform == "win32":
                ...
                try:
                    for entry in sorted(os.listdir(base)):
                        ...
                        if os.path.isdir(full):
                            roots.append((full, entry if len(entry) <= 10 else entry[:9] + "…"))
            ...
            for path, label in roots:
                ...
                seen.add(path)
                ctk.CTkButton(
                    parent, text=label, width=44, height=22,
                    fg_color=BG_COLOUR, hover_color=BORDER_COLOUR,
                    text_color=TEXT_COLOUR, font=make_font(10),
                    command=lambda p=path: self._win_navigate(p),
                ).pack(side="left", padx=2)
    ...
    def _fetch_dev_listing(self, path):
        ...
        if rc != 0:
            self._ui(self._populate_dev_lb, None,
        ...
        self._ui(self._populate_dev_lb, entries, None)
    ...
    def _transfer_worker(self, fn, args):
        try:
            ...
            self._ui(self._set_status, f"Transfer error: {e}", "error")
        finally:
            self._ui(self._finish_transfer)
    ...
    def _do_push(self, local_files, remote_dir):
        ...
        for i, local in enumerate(local_files):
            ...
            self._ui(self._progress_var.set, i / total)
            ...
            if timed_out or self._cancel_flag:
                ...
                self._ui(self._set_status, msg, kind)
                return
            ...
            if rc != 0:
                ...
                self._ui(self._set_status, f"Push failed: {msg}", "error")
                logger.error(f"Push failed {local}: {msg}")
                return

            done += 1
            self._ui(self._progress_var.set, done / total)
            logger.info(f"Pushed: {local} -> {remote}")

        self._ui(self._set_status,
                          f"Pushed {total} file(s) successfully", "success")
        self._ui(self._progress_var.set, 1.0)
    ...
    def _do_pull(self, remote_files, local_dir):
        ...
        for i, remote in enumerate(remote_files):
            ...
            if rc != 0:
                ...
                msg = stderr.strip() or stdout.strip() or f"exit {rc}"
                self._ui(self._set_status, f"Pull failed: {msg}", "error")
    ...
    def _apply_dark_titlebar(self):
        self.window.update_idletasks()
        apply_dark_titlebar(self.window)   # no-op off Windows
